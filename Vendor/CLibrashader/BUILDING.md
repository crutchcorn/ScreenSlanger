...
ScreenSlanger uses **0.12.0-screenslanger.1**, built from upstream tag
[`librashader-v0.12.0`](https://github.com/SnowflakePowered/librashader/tree/librashader-v0.12.0)
with the two patches shipped beside this file. The upstream C header and ABI are
unchanged. This build enables only the Metal runtime.

The application includes the complete upstream source archive, its original
`Cargo.lock`, both applied patches, and the MPL-2.0 license in
...
`~/Library/Application Support/ScreenSlanger/Tools/librashader/0.12.0-screenslanger.1/`.
The official `0.12.0` installation is preserved. Normal Xcode builds only verify,
...
and both patches against the recorded distribution; `BUILD-INFO.txt` records
...
and the upstream dependency lockfile remain unchanged.
